...
try {
  ...
  await page.getByText('Тип тренировки', { exact: true }).click();
  assert.equal(await page.evaluate(() => document.activeElement.id), 'workout-type-select');
  assert.equal(await page.locator('[role=combobox]').getAttribute('aria-expanded'), 'true');
  await page.locator('[role=combobox]').fill('rnn');
  await page.locator('[role=combobox]').press('ArrowDown');
  const activeId = await page.locator('[role=combobox]').getAttribute('aria-activedescendant');
  ...
  assert.equal(await page.locator('#time-inputs').isVisible(), true);
  assert.equal(await page.locator('#strength-inputs').isVisible(), false);
  await page.locator('[role=combobox]').press('ArrowDown');
  await page.locator('[role=combobox]').press('Escape');
  assert.equal(await page.evaluate(() => document.activeElement.id), 'workout-type-select');
  assert.equal(await page.locator('[role=combobox]').getAttribute('aria-expanded'), 'false');
