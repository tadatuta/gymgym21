...
try {
  ...
  const fetchCheck = `const ready=await fetch('http://127.0.0.1:8788/ready'); if(ready.status!==200 || !(await ready.json()).ok) throw Error('readiness failed'); const h=await fetch('http://127.0.0.1:8788/health'); if(h.status!==200 || !(await h.json()).ok) throw Error('health failed'); const r=await fetch('http://127.0.0.1:8788/api/auth/get-session'); if(r.status!==200 || await r.text()!=='null') throw Error('auth session failed '+r.status);`;
