...
try {
  ...
  await waitFor(() => output.includes("[server] listening { host: '127.0.0.1', port: 49873 }") && output.includes('Local:'), 'root npm dev clean-dist/env precedence');
  ...
  await waitFor(() => output.includes("[server] listening { host: '127.0.0.1', port: 49872 }"), 'workspace uses root env');
  await stop();
  ...
  await waitFor(() => output.includes("[server] listening { host: '127.0.0.1', port: 49873 }"), 'exported env without file');
  await stop();
