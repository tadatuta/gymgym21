...
Публичный endpoint ограничен по IP: `RATE_LIMIT_PUBLIC_WINDOW_MS=60000`, `RATE_LIMIT_PUBLIC_MAX=60`, `RATE_LIMIT_PUBLIC_MAX_CONCURRENT=4`; превышение возвращает `429` и `Retry-After`. Пока счётчики находятся в памяти процесса; при нескольких репликах лимит действует отдельно в каждой. Общий PostgreSQL store запланирован в O05. AI читает не более `AI_MAX_EXERCISE_COUNT` типов и `AI_MAX_RECENT_LOGS` подходов непосредственно через SQL LIMIT; неиспользуемые сессии тренировок не читает. Исторические агрегаты по-прежнему сканируют историю в PostgreSQL на промахе кэша, но не загружают её целиком в Node.js. Воспроизводимые планы и ограничения: [audit/public-query-performance.md](audit/public-query-performance.md).
