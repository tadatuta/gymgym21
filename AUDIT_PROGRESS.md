...
| A04 | P1 — Сетевые операции не привязаны к неизменной идентичности аккаунта | Завершено | audit_04_account_context / текущий коммит | Node22: typecheck, client lint, client tests 40/40; server+изолированная PostgreSQL 41/41 без skipped. Account-scoped sync instance, generation/abort, guards AI/public cache и auth late401/restore; общий auth channel без rebroadcast на session reads; обязательный expected-storage header/CORS. Реальный auth resolver с signed cookie B отвергает A/missing identity для sync/AI до effects, обе snapshots неизменны; B sync проходит. Browser multi-tab E2E остаётся T05. |
