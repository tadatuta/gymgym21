> Исторический отчёт проверки от 6 сентября 2026 на `c78452f`. Текст ниже сохранён без изменений и описывает состояние до исправлений. Актуальный статус на 8 сентября 2026 — [AUDIT_PROGRESS.md](AUDIT_PROGRESS.md); текущая эксплуатация — [runbook](docs/runbook.md). `audit/reproduce*.mjs` в T06 заменены entrypoints текущих regression suites; описание их старого поведения ниже сохранено как история.
