> Исторический отчёт проверки от 6 сентября 2026 на `c78452f`. Текст ниже сохранён без изменений и описывает состояние до исправлений. Актуальный статус на 8 сентября 2026 — [AUDIT_PROGRESS.md](AUDIT_PROGRESS.md); текущая эксплуатация — [runbook](docs/runbook.md). Исторические `audit/reproduce*.mjs` не являются текущими CI-проверками; их переработка — пункт T06.
