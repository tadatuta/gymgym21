> Исторический план, перенесён в архив 8 сентября 2026. Это не инструкция для текущей версии; содержание ниже сохранено без изменений. Действующая [архитектура](../architecture.md) и [runbook](../runbook.md).
...
- rollback на старую архитектуру поддерживать не нужно;
- после cutover legacy-код и legacy runtime должны удаляться сразу, без compatibility-слоёв и fallback-режимов.
...
- удалить legacy JSON из runtime-контура;
- не использовать их больше нигде в приложении;
- не монтировать storage volume в контейнер backend.

## 15. Что должно быть удалено из репозитория после завершения
...
Дополнительно нужно удалить:

- любые feature flags, включающие старый storage;
- временные compatibility DTO, если они были заведены на период переписывания;
- промежуточные adapter-слои, если они нужны были только для переноса со старой модели.
...
- в проекте не осталось rollback-веток и legacy fallback-путей;
- старая storage-архитектура физически удалена из кодовой базы.

## 17. Короткая архитектурная формула миграции
...
- legacy storage не консервируется, а удаляется сразу после перехода.
